...
module UserModule =
    ...
    let validateId id =
        if id >= 0 then
            ...
            Error(
                { UserError.field = UserField.Id
                  value = id
                  error = sprintf "Invalid id. Value '%d' must be a positive number" id }
            )
    ...
    let validateEmail email =
        ...
        if regex.IsMatch email then
            ...
            Error(
                { UserError.field = UserField.Email
                  value = email
                  error = sprintf "Invalid email. Value '%s' must use email format in lowercase" email }
            )
